##Data Backgrounding 
Do successful (winning) Big Ten teams rely on their first-year players more than others? 

```{r}
library(tidyverse)
library(dplyr)
```


```{r}
ncaa_players_2023 <- read_csv ("https://raw.githubusercontent.com/dwillis/NCAAWomensSoccerData/main/data/ncaa_womens_soccer_playerstats_2023.csv")
```


```{r}
head(ncaa_players_2023)
```
```{r}
ncaa_players_2023|>
  group_by(team, yr) |>
  summarise(
    total = n()
  ) 
```

```{r}
ncaa_players_2023|>
  group_by(team, yr) |>
  summarise(
    total = n(),
    mean_goals = mean(goals)
  ) 
```

Basically what I want to see is from each team, the avg amount of goals for each yr by team. I wouldn't do it for the NCAA entirely because 1. Looking at the teams separately tells more of a story (we can see if one team relies on one year more than another) 2. We can easily narrow it down to the big 10 teams. 


```{r}
byyr <- ncaa_players_2023 |>
  mutate(
    goalspergame = goals/games, 
    goals <- goals %>% replace(is.na(.), 0)) |>
  filter(
    yr == "Fr" |
    yr == "Sr" |
    yr == "So" | 
    yr == "Jr") |>
  filter(
    team == "Ohio St. Buckeyes" |
    team == "Michigan St. Spartans" |
    team == "Penn St. Nittany Lions" |
    team == "Rutgers Scarlet Knights" |
    team == "Maryland Terrapins, Terps" |
    team == "Michigan St. Spartans" |
    team == "Indiana Hoosiers" |
    team == "Iowa Hawkeyes" |
    team == "Wisconsin Badgers" |
    team == "Minnesota Golden Gophers" |
    team == "Nebraska Huskers" |
    team == "Northwestern Wildcats" |
    team == "Illinois St. Redbirds" |
    team == "Purdue Boilermakers"
  )
```
Here I am creating a graph to demonstrate the relationship between each year for each of the Big 10 teams.The graph displays the amount of goals each of the years makes in comparison to one another on each team. 
```{r}
ggplot() + 
  geom_bar(
    data=byyr, 
    aes(x=team, weight=goalspergame, fill=yr)) + 
  coord_flip()
```

So now that we have a graph that represents the amount of goals each year from each team, I think this can more accurately represent the data I would like to display. Lets see how each year contributes to assists... 


##assists by year
```{r}
ncaa_players_2023|>
  group_by(team, yr) |>
  mutate(assists <- assists %>% replace(is.na(.), 0)) |>
  summarise(
    total = n()
  ) 
```


```{r}
assistsbyyr <- ncaa_players_2023 |>
  mutate(
    assistspergame = assists/games, 
    goals <- goals %>% replace(is.na(.), 0)) |>
  filter(
    yr == "Fr" |
    yr == "Sr" |
    yr == "So" | 
    yr == "Jr") |>
  filter(
    team == "Ohio St. Buckeyes" |
    team == "Michigan St. Spartans" |
    team == "Penn St. Nittany Lions" |
    team == "Rutgers Scarlet Knights" |
    team == "Maryland Terrapins, Terps" |
    team == "Michigan St. Spartans" |
    team == "Indiana Hoosiers" |
    team == "Iowa Hawkeyes" |
    team == "Wisconsin Badgers" |
    team == "Minnesota Golden Gophers" |
    team == "Nebraska Huskers" |
    team == "Northwestern Wildcats" |
    team == "Illinois St. Redbirds" |
    team == "Purdue Boilermakers"
  )
```

```{r}
ggplot() + 
  geom_bar(
    data=assistsbyyr, 
    aes(x=team, weight=assistspergame, fill=yr)) + 
  coord_flip()
```
Here we can see how each year contributes to to assists for the season, in comparison with the each year and each team. 
Questions: Where in this data do you see a story? 
Are Freshman a significant contributor to a teams success?/Does this data need more support to answer that? 








```{r}
assistsbyyr |>
  group_by(team, yr) |>
  summarise(
    sum = (assists)
  ) |>
  filter(
    yr == "Fr")
```

Percentage of a team's goals that first-year players account for. That means you'll need to create a dataframe called team_goals by grouping by team and summing up the goals as total_goals, then do the same but filtering only on first-year players, creating a dataframe called first_year_goals with a column called total_first_year_goals. Then you'd join the two dataframes like this:
combined <- team_goals |> left_join(first_year_goals, join_by(team))
Next, you'd replace any NAs in total_first_year_goals with 0 and calculate the percentage using mutate().

```{r}
team_goals <- byyr |>
  group_by(team)|>
  summarise (goals/"Fr" == total_goals)
```

