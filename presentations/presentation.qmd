...
```{r}
top10fr <- team_fr_goals_2023 |>
  filter(total_first_year_goals>=13) |>
  arrange(desc(total_first_year_goals))
```
...
team_fr_pct <- combined |>
  ...
  mutate(fr_pct <- fr_pct %>% replace(is.na(.), 0))|>
  arrange(desc(fr_pct))
```
...
ggplot() + 
  geom_bar(
    data=byyr, 
    aes(x=reorder(team, goalspergame), weight=goalspergame, fill=yr)) + 
  coord_flip()
```
