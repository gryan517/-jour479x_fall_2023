...
```{r}
nbaplayers <- read_csv("~/GitHub/-jour479x_fall_2023/exercises/aggregates/data/nbaadvancedplayers2223.csv")
```
**Answer** DESCRIBE THE RESULTS HERE
There are 679 columns for 679 players and 27 rows in this data. The column titles are listed after chr and abl. Where chr = player name.
...
**Answer** DESCRIBE THE RESULTS HERE
In the position column we see the positions are in alphabetical order, instead of another order such as sequential by the total. 
...
A complicating factor I notice are the repeating totals with PG-SG, SF_SG and SG-PG and then with PF-C ad PF-SF. I'm not too sure how I would resolve this. What I'm questioning out of the positions with the repeating numbers, which one comes first? 
...
**ANSWER** 
I think there can be a story with the similar stats of A PF and SF. All members have responsibilities that work with one another to reach the teams goals, but these two positions in particular seem to be a push and pull situation, where as in most teams SF acts as as a defender and PF aids in assists. These two positions are powerfully working with one another and I think there can be a story there where these positions play a similar amount on average and max. I would propose the question, as a PF and SF do you have to be a highly reliable and consistent player (more so than other positions)?