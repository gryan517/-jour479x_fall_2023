...
The not applicable/ black data would make it difficult to make year-over-year comparisons. Especially with Appalachian State.  
...
FIU had an inf amount of change due to dividing by 0 number of attendance. Hawaii is next since it is a major plane ride away, COVID restrictions and possibly due to asking tourists to refrain from travelling. Something that is also interesting is MWC being in the top 9 3 times, that's a lot!


## Back to women's soccer

Let's turn back to the womens soccer match data from this season.
...
**Answer** The results that stand out the most to me is the percentage difference to the California Golden Bears with 60%. Compared to the range of percentages this is definitely an outlier where there are many teams in the 40% range, 30% and so on. There is also a point in the data where many teams have a goal percentage of 33.33. I'm not sure how I would narrow down the data. My instinct is to  re-arrange the data in desc order for total_on_goal but I"m not sure what I can get from this. Possibly if shooting more guarantees more total goals?
```{r}
matches_2023 |> 
  group_by(team) |> 
  summarise(
