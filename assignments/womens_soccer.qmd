...
```
```{r}
matches_2023 <- read_csv("https://github.com/dwillis/NCAAWomensSoccerData/blob/eff07032d899042b7777527f3fe45b448c46468f/data/ncaa_womens_soccer_matchstats_2022.csv")
```
```{r}
head(matches_2022)
...
```{r}
matches_2023|>
  group_by("Maryland Terrapins, Terps")|>
  summarise(
    sum_goals = (sum (so_g)/sum(so_g,sh_att))*100
  )
...
```{r}
head(matches_2022)|>
  group_by("Maryland Terrapins, Terps", date)|>
  summarise(
    sum_goals = (sum (so_g)/sum(so_g,sh_att))*100
    )

#cannot figure out how to isolate the first four dates, also don't know what happened to the data. i used the following commands before and after going through other questions i get this error.
```
...
#I'm having the same issue with the previous question, but believe this would be the command minus this error. 
```
...
```{r}
matches_2023|>
  group_by("Maryland Terrapins, Terps", team)|>
  summarise(
    corners_sd = sd(corners)
   )
```
